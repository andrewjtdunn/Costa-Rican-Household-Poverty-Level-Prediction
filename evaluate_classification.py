...
def evaluate_classification(y_pred, y_true, l=[1,2,3,4], cm=False, return_vals=False):
    ...
    eval_dict['precision'] = precision_score(y_true, y_pred, average= None, zero_division=1)
    ...
    if len(eval_dict['precision']) == 2:
        print("Precision:", 'Experiencing Poverty:', eval_dict['precision'][1].round(2), 'Not Experiencing Poverty:', 
            eval_dict['precision'][0].round(2),"\n")
    else:
        print("Precision:", 'Class 1:', eval_dict['precision'][0].round(2), 'Class 2:', 
            eval_dict['precision'][1].round(2), ' Class 3:', eval_dict['precision'][2].round(2))
        if len(eval_dict['precision']) == 4:
            print('           Class 4:', eval_dict['precision'][3].round(2),"\n")
    
    if len(eval_dict['recall']) == 2:
        print("Recall:", 'Experiencing Poverty:', eval_dict['recall'][1].round(2), 'Not Experiencing Poverty:', 
            eval_dict['recall'][0].round(2),"\n")
    else:
        print("Recall:", 'Class 1:', eval_dict['recall'][0].round(2), 'Class 2:', 
            eval_dict['recall'][1].round(2), ' Class 3:', eval_dict['recall'][2].round(2))
